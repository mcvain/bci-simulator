...
eeglab; 
output_param_filename = 'rotated2D.prm'; 
sampling_rate = 250; 
baseline_method = 3; 
...
for axis = 1:2 
    ...
    for dir = 1:2  % 2 directions in a given axis. 
        ...
        for tr = 1:length(trials)
            ...
            if length(eeg_control) >= 1  % If using pwelch, feedback control period must be sufficiently long, but if using bandpower, this conditional is unnecessary.   
                % Spatial filter
                ...
                C3_data_control = double(C3_data_control);
                C3_data_baseline = double(C3_data_baseline); 

                % (insert some filter stuff here if needed...) 

                C3_control_alpha = bandpower(C3_data_control, sampling_rate, [8 12]); 
                C3_baseline_alpha = bandpower(C3_data_baseline, sampling_rate, [8 12]); 
                C4_control_alpha = bandpower(C4_data_control, sampling_rate, [8 12]); 
                C4_baseline_alpha = bandpower(C4_data_baseline, sampling_rate, [8 12]); 
...
function [f, power] = EEG_power(EEG_timetrace, fs)

    realx = EEG_timetrace(1:end);
y = fft(realx);
...
figure; hold on; plot(f,power);
xlabel('Frequency')
ylabel('Power')
xlim([0 50]); 
end

function y = customNotch(x, fs) 
